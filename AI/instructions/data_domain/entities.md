...
Entities are **Eloquent models** that represent database tables and handle data persistence. Entities delegate complex operations to **services** for business logic and data access.
...
// ✅ Good: Use service (or Scope for reusability)
class UserService
{
    public function getActiveSubscribersWithPendingOrders(): Collection
    {
        return User::query()
            ...
            ->whereHas('orders', fn($q) => $q->pending())
            ->get();
    }
}
...
5. **Delegate complexity** - To services
